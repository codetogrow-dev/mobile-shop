...
import { listSuppliersPage } from '@/api/suppliers';
import { SupplierRow } from './supplier-row';
import type { SupplierListRow, PartyListSort } from '@/types/app';

const PAGE_SIZE = 10;
const SORT_LABELS: Record<PartyListSort, string> = {
  spent:   'Most purchased',
  visits:  'Most batches',
  recent:  'Recent purchase',
  balance: 'You owe most',
  name:    'Name (A–Z)',
};
...
export default function SuppliersView() {
  ...
  } = usePaginatedList<SupplierListRow>({
    queryKey: QK.suppliers.page(committedSearch, sort),
    fetchPage: (offset, limit) =>
      listSuppliersPage({ search: committedSearch, sort, offset, limit }),
    pageSize: PAGE_SIZE,
  });
  ...
  return (
    <View style={[styles.container, { paddingTop: insets.top }]}>
      <View style={styles.header}>
        ...
        <View style={{ flex: 1 }}>
          <ThemedText type="h2">Suppliers</ThemedText>
        ...
        <TouchableOpacity
          style={styles.addBtn}
          onPress={() => router.push('/(app)/add-supplier' as any)}
          activeOpacity={0.8}
        >
      ...
      <FlashList
        ...
        renderItem={({ item, index }) => (
          <SupplierRow
            ...
            onPress={() => router.push(`/(app)/supplier/${item.id}` as any)}
          />
        )}
          ...
          <View style={styles.empty}>
            ...
            <ThemedText type="body" color={colors.textSecondary}>
              {isLoading ? 'Loading suppliers…' :
                committedSearch ? `No suppliers match "${committedSearch}"` : 'No suppliers yet'}
            </ThemedText>
            {!isLoading && !committedSearch && (
              <TouchableOpacity onPress={() => router.push('/(app)/add-supplier' as any)}>
                <ThemedText type="caption" color={colors.primary500}>+ Add your first supplier</ThemedText>
              </TouchableOpacity>
            )}
          </View>
        )}
