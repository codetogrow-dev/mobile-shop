...
import { supplierSchema, type SupplierFormValues } from '@/types/app';
...
export default function AddSupplierView() {
  ...
  const mutation = useMutation({
    mutationFn: (values: SupplierFormValues) => createSupplier({ ...values, tenant_id: tenantId }),
    onSuccess: () => {
    ...
    <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <View style={[styles.container, { paddingTop: insets.top }]}>
        <View style={styles.header}>
          ...
          <ThemedText type="h3">Add Supplier</ThemedText>
          <View style={{ width: 36 }} />
        </View>
          ...
          {mutation.error && (
            <View style={styles.errorBanner}>
              ...
              <ThemedText type="caption" color={colors.danger}>
                {(mutation.error as any)?.message ?? 'Failed to create supplier'}
              </ThemedText>
            </View>
          )}

          <Card>
            <ThemedText type="h4" style={styles.sectionTitle}>Supplier details</ThemedText>
            ...
            <Controller
              ...
              render={({ field }) => (
                <Input
                  ...
                  placeholder="Office / Warehouse / Area"
                  value={field.value ?? ''}
                  onChangeText={field.onChange}
                  multiline
                  numberOfLines={2}
